...
export async function register(req: Request, res: Response) {
  const { name, email, phone, password, role } = req.body as Record<string, string>;
  ...
  const accessToken = signAccessToken({ sub: String(user._id), role: user.role as any });
  const refreshToken = signRefreshToken({ sub: String(user._id), role: user.role as any });
...
export async function login(req: Request, res: Response) {
  const { emailOrPhone, password } = req.body as Record<string, string>;
...
export async function refresh(req: Request, res: Response) {
  ...
  try {
    ...
    if (!user || !user.refreshTokenHash) return res.status(401).json({ message: "Invalid token" });
    const match = await bcrypt.compare(token, user.refreshTokenHash);
    if (!match) return res.status(401).json({ message: "Invalid token" });
    const accessToken = signAccessToken({ sub: String(user._id), role: user.role as any });
    const newRefreshToken = signRefreshToken({ sub: String(user._id), role: user.role as any });
