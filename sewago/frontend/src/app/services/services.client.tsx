...
export default function ServicesClient({ initialItems, initialTotal, initialPerPage }: { initialItems: Service[]; initialTotal: number; initialPerPage: number; }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    initialData: { items: initialItems, total: initialTotal, perPage: initialPerPage },
  });
  ...
  return (
    <>
      ...
      {isLoading && initialItems.length === 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" aria-busy>
      ...
      {data?.items?.length ? (
        ...
        <div className="border rounded p-6 bg-slate-50" role="status" aria-live="polite">
